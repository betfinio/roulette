...
const TableItem: React.FC<TableItemProps> = ({
	...
	const addChip = (position: string, relatedNumbers: number[]) => {
	...
	const handleClick = (position: string, relatedNumbers?: number[]) => {
	...
	const renderChip = (position: string) => {
		const chipsForPosition = chipsOnThisNumber.filter((chip) => chip.position === position);

		// Retornar todas as fichas com z-index dinâmico
		return chipsForPosition.map((chip, index) => {
			const zIndex = chipCount + index + 1; // z-index incremental baseado na contagem global e no índice

			return (
				<div
					key={`${chip.number}-${chip.position}-${index}`}
					className="pointer-events-none absolute"
					style={{ zIndex }} // Aplicar z-index dinâmico baseado na contagem de fichas
				>
