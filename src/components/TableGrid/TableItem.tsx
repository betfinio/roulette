...
const TableItem: React.FC<TableItemProps> = ({
  ...
  const chipsOnThisNumber = placedChips.filter(
    (chip) => chip.number === number
  );

  const renderChip = (position: PositionType) => {
    const chipsForPosition = chipsOnThisNumber.filter(
        ...
        >
          ...
          >
            ...
            <text
              ...
              dy=".3em"
              fill="#000"
              className="text-[37px] font-semibold"
            >
  ...
  return (
    ...
    >
      ...
      <div
        className={`${isRangeButton ? "side-item-v-text" : "number-display"}`}
      >
