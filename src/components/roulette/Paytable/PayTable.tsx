...
const Paytable: FC<IPaytableProps> = ({ onClose }) => {
	...
	return (
		<div className={'bg-card games rounded-lg p-4 w-full text-foreground relative'}>
			...
			/>
			<h2 className={'text-accent-secondary-foreground font-semibold text-lg'}>{t('payTable.paytable')}</h2>
			<div className={'w-full grid grid-cols-4 gap-1 my-4'}>
				...
				{limits.map((limit, i) => (
					<div className={'col-span-4  grid grid-cols-4'} key={i}>
						...
						<span className={'text-accent-secondary-foreground'}>{limit.payout}x</span>
			...
			<div className={'mt-5 text-sm flex justify-end'}>
				<a
					className={'flex gap-1 underline hover:text-accent-secondary-foreground duration-300'}
