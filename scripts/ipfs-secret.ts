			...
			if (typeof config === 'object' && config !== null) {
				...
				for (const [envName, envConfig] of Object.entries(config)) {
					if (envName !== '__env' && typeof envConfig === 'object' && envConfig !== null) {
						const ipnsKey = (envConfig as Record<string, string>).ipnsPublicKey;
						if (ipnsKey) {
 ...
 */
async function fetchIPFSContent(name: string): Promise<Record<string, string | number | boolean | object | null>> {
	try {
...
// biome-ignore lint/suspicious/noExplicitAny: we need to ignore this because we are using any
